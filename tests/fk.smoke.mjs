// Headless browser smoke test for farkle: lobby → farkle → play whole games by tapping → leave.
// Usage: node tests/fk.smoke.mjs [games]
...
await page.click('button.hotspot:has-text("Farkle"), .door:has-text("Farkle")');
...
await page.waitForSelector('.table-screen.fk');

const GAMES = +process.argv[2] || 1;
let games = 0, rolls = 0, banks = 0, picks = 0, leaveClicked = false;
const seen = new Set();
const shot = async (k) => { if (!seen.has(k)) { seen.add(k); await page.screenshot({ path: `tests/shots/fk-${k}.png` }); } };
const t0 = Date.now();
while (Date.now() - t0 < 420000) {
  ...
  const rollBtn = await page.$('.actionbar:not(.hidden) button.act:has-text("Roll"):not([disabled])');
  const pick = await page.$('.actionbar:not(.hidden) button.act:has-text("Pick for me")');
  if (pick) {
    const sel = await page.$$('.die.sel');
    if (!sel.length) { await pick.click(); picks++; await page.waitForTimeout(120); await shot('pick'); continue; }
    const bankBtn = await page.$('.actionbar:not(.hidden) button.act:has-text("Bank"):not([disabled])');
    const turnTxt = await page.$eval('.fk-turn', (e) => e.textContent).catch(() => '');
    const turn = +(turnTxt.replace(/[^0-9]/g, '') || 0);
    if (bankBtn && (turn >= 350 || Math.random() < 0.3)) { await bankBtn.click(); banks++; continue; }
    if (rollBtn) { await rollBtn.click(); rolls++; continue; }
    if (bankBtn) { await bankBtn.click(); banks++; continue; }
  } else if (rollBtn) { await rollBtn.click(); rolls++; await shot('roll'); continue; }
  if (!seen.has('farkle') && await page.$('.fk-msg.farkle')) await shot('farkle');
...
await browser.close();
server.close();
console.log(`farkle: ${games} games, ${rolls} rolls, ${picks} picks, ${banks} banks; bank now ${bankTxt}; atTable=${JSON.stringify(save.atTable)}`);
