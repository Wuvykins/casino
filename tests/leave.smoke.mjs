...
const results = [];
for (const [game, ready] of [["Texas Hold'em", '.actionbar:not(.hidden) button.act, .nextbar:not(.hidden) button'], ['Blackjack', '.actionbar:not(.hidden) button.act'], ['Cribbage', '.cb-hand .pcard'], ['Farkle', '.actionbar:not(.hidden) button.act']]) {
  ...
  const mid = await page.waitForSelector('.modal button:has-text("Leave now")', { timeout: 5000 }).catch(() => null);   // between hands there's nothing to ask
  const note = mid ? await page.$eval('.modal', (e) => e.innerText.replace(/\s+/g, ' ')) : '(between hands: left straight away)';
  if (mid) await page.click('.modal button:has-text("Leave now")');
